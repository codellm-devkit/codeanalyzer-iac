"""Console-script entry point: run the bundled ``caniac`` binary.

``pip install codeanalyzer-iac`` installs a ``caniac`` launcher (see
...
def main() -> "int | None":
    """Exec the bundled ``caniac`` binary with the current argv (minus argv[0])."""
    binary = str(bin_path())
    args = [binary, *sys.argv[1:]]

    if os.name == "posix":
        # Replace this process with the binary: no extra Python process lingers,
        # and signals/exit codes are handled by caniac directly.
